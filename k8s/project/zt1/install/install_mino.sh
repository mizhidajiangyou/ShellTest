...
if [ "${helm_mode}" == "yaml_apply" ]; then
  ...
  --set auth.rootPassword=$(getImagesConf $service_name 'password') \
  --set auth.rootUser=$(getImagesConf $service_name 'user') \
  --set persistence.size=$(getImagesConf $service_name 'size') \
