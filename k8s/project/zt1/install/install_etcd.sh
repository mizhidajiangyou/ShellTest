...
elif [ "${helm_mode}" == "helm_delete" ]; then
  helm_cmd="helm delete"
  if ! helm delete "$service_name" -n "${namespace}"; then
    exit 1
  else
    exit 0
  fi
...
if [ "${helm_mode}" == "yaml_apply" ]; then
  ...
  conf_cmd="\
  --set image.registry='' \
  --set image.repository=$(getImagesPart $service_name 'image' 1) \
  --set image.tag=$(getImagesPart $service_name 'image' 2) \
  --set replicaCount=$(getImagesConf $service_name 'replica_count') \
  --set persistence.storageClass=$(getImagesConf 'k8s' 'storage_class') \
